...
class ViewsTest < Minitest::Test
  ...
  class MockApp
    ...
    def push_view(view); end
    def pop_view; end
    def trigger_connect; end
    def trigger_refresh; end
  end
  ...
  def test_main_view_render
    ...
    with_test_terminal do
      ...
      header_content = header.text.map { |l| 
        l.is_a?(String) ? l : l.spans.map { |s| s.respond_to?(:content) ? s.content : s.to_s }.join 
      }.join
  ...
  def test_debug_view_render
    ...
    # Mock session
    @app.session = Ksef::Session.new(token: 'secret-token', valid_until: 'tomorrow')
